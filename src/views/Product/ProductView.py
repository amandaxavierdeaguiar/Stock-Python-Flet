...
class ProductView(SharedControls):
    ...
    # Adicionar novo produto - Bye, Amanda
    @classmethod
    def add_product(cls, product):
        entity = ProductDto.model_validate(product)
        cls.ctrl_product.add(entity, cls.user) #modificado

    @classmethod
    def update_product(cls, product, selected):
        entity = ProductDto.model_validate(product)
        if entity.price == selected.price:
            cls.ctrl_product.update(entity, cls.user)
        else:
            cls.ctrl_product.update_price(entity, cls.user)

    @classmethod
    def add_supplier(cls, supplier):
        entity = SupplierDto.model_validate(supplier)
        cls.ctrl_supplier.add(entity, cls.user)

    # Ver se a password é igual e se tem o mesmo usuário
