...
from app.app_style import button, text_blue
...
class AuthLogin(SharedControls):
    ...
    def button_submit(cls, var_on_click):
        return ft.ElevatedButton(
            text="Entre", **button(), width=140, on_click=var_on_click, data="login"
        )  # disabled=True,

    @classmethod
    def create_button_signup(cls, data, var_on_change=None):
        return ft.ElevatedButton(
            **button(), text="Cadastra-se", width=140, on_click=var_on_change, data=data
        )
